...
/**
 * 將 ISO 週次字串 (例如 "2025-W27") 轉換為日期範圍字串。
 * @param {string} weekString - "YYYY-Www" 格式的字串。
 * @returns {string} - "YYYY/MM/DD ~ YYYY/MM/DD" 格式的日期範圍字串。
 */
export function getWeekDateRange(weekString) {
    try {
        ...
        // 找到1月4日是星期幾 (0=Sun, 6=Sat)，並計算出第一週的星期一
        const dayOfWeek = jan4.getUTCDay() || 7; // Sunday is 0, make it 7
        ...
        // 如果格式不對或計算出錯，返回原始字串
        console.error("解析週次字串時發生錯誤:", weekString, e);
        return weekString;
    }
}
